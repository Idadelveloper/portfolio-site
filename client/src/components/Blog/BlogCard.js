...
export default function BlogCard(props) {
    const gaEventTracker = useAnalyticsEventTracker(`Blog card - ${props.post?.title}`);
    return (
        <motion.div layout key={props.key} className="w-full max-w-md mx-auto rounded-3xl overflow-hidden shadow-lg mb-20">
            ...
            <div className="px-6 pt-4 pb-2 flex justify-between">
                ...
                <Link to={props.post?.slug} onClick={()=>gaEventTracker('read more')} className="bg-blue-500 hover:bg-blue-700 text-white font-semibold py-2 px-4 text-sm rounded-full mb-2 card-read-more flex items-center">Read more</Link>
            </div>
        </motion.div>
    )
    }
